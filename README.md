...
All functionality is within `intel_hex_tool.py`. Requires at least python 3.6 to run the script and 3.8 to run certain dev tools, no 3rd party dependencies. If used in a venv, can be called with `iht`.
...
Optionally install or manually run the precommit hooks: (requires at least python 3.8)
```bash
prek install
prek run --all-files
```
